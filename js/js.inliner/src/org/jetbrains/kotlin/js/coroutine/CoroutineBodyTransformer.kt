...
class CoroutineBodyTransformer(val program: JsProgram, val scope: JsScope, val throwFunctionName: JsName?) : RecursiveJsVisitor() {
    ...
    override fun visitBreak(x: JsBreak) {
        ...
        updateExceptionHandler(targetTryDepth)
    ...
    private fun updateExceptionHandler(targetTryDepth: Int, successor: CoroutineBlock? = null) {
        ...
        val relativeFinallyDepth = relativeFinallyPath(targetTryDepth)
        if (relativeFinallyDepth.isNotEmpty()) {
            currentStatements += updateFinallyPath(relativeFinallyDepth + successor.singletonOrEmptyList())
        }
    }
    ...
    override fun visitReturn(x: JsReturn) {
        ...
        updateExceptionHandler(0, successor = if (isInFinally) returnBlock else null)
        ...
        if (isInFinally) {
            ...
            returnBlock.statements += JsReturn()
        }
        else {
            currentStatements += x
        }
