...
public class DescriptorResolver {
    ...
    public List<KotlinType> resolveSupertypes(
        ...
        ClassDescriptor serializableClassDescriptor = KSerializationUtil.getSerializableClassDescriptor(classDescriptor);
        if (serializableClassDescriptor != null && !containsKSerializer(supertypes)) {
            supertypes.add(builtIns.getKSerializerType(serializableClassDescriptor.getDefaultType()));
        }
    ...
    private static boolean containsKSerializer(Collection<KotlinType> result) {
        for (KotlinType type : result) {
            if (KotlinBuiltIns.isKSerializer(type)) {
                return true;
            }
        }
        return false;
